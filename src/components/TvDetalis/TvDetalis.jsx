...
export default function TvDetalis() {
  ...
  async function getdetails() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US`);
    setmedia(data);
    setdetailsloading(false)
  }
  async function getkeywords() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}/keywords?api_key=f708973a9ad44b00b93dd5ac49f93aa2`);
    setmediakeyword(data.results);
    setkeywordsloading(false)
  }
  async function getcredits() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}/credits?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US`);
    setcreditscast(data.cast.splice(0, 15));
    setcreditscrew(data.crew.splice(0, 15));
    setcreditsloading(false)
  }
  async function getreviews() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}/reviews?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US`);
    setreviews(data.results.slice(0, 5));
    setreviewsloading(false)
  }
  async function getrecommendations() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US`);
    setrecommendations(data.results.slice(0, 4));
    setrecommendationsloading(false)
  }
  async function getsimilar() {
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${id}/similar?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US`);
            ...
            <div className="container position-relative py-5">
              <div className="row mt-5 pt-2">
                ...
                <div className="col-md-9 ps-5 position-relative ">
                  <h3 className='fw-bold' >{media.name} <a href={media.homepage} target="_blank" rel="noreferrer"><i className="fa-solid fa-share-from-square fs-5 ms-2 text-white" ></i></a></h3>
                  <span className="fs-s">{media.first_air_date} .</span>
                  ...
                  <p className='fs-sm'>{media.overview}</p>
                  <ul className='d-flex flex-wrap'>
                    {(media.created_by.length > 0) ? <li className="d-flex flex-column fs-sm mb-2 me-5"><span className='fw-bold fs-6'>Creator</span>{media.created_by[0].name}</li> : ""}
                    ...
                    <li className="d-flex flex-column fs-sm mb-2 me-5"><span className='fw-bold fs-6'>Type</span>{media.type}</li>
            ...
            <div className="mt-5 px-2">
              ...
              {recommendations.length > 0 ? <>
                ...
                {recommendations.map((e, i) => {
                  return <Link key={i} to={`/tvdetalis/${e.id}`} className="text-decoration-none text-white">
              ...
              {similar ? <>
                <h5 className="fw-bold ms-2 mb-4 pb-3 mt-5">similar tv show</h5>
                {similar.map((e, i) => {
                  return <Link key={i} to={`/tvdetalis/${e.id}`} className="text-decoration-none text-white">
                    <img src={(e.backdrop_path ? imgPath + e.backdrop_path : nullphoto)} alt="moviephoto" className='backdrop_ rounded-2 nullphoto' />
                    <div className="d-flex justify-content-between align-items-center mt-2 mb-4">
