...
export default function Tvshow() {
  ...
  async function getdata(pagenum = 1) {
    ...
    let { data } = await axios(`https://api.themoviedb.org/3/tv/${type}?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US&page=${pagenum}`);
  ...
  async function getsearchdata(query) {
    setmedia(null)
    setloading(true)
    let { data } = await axios(`https://api.themoviedb.org/3/search/tv?api_key=f708973a9ad44b00b93dd5ac49f93aa2&language=en-US&query=${query}&page=1&include_adult=false`);
    let res = data.results;
    setcaursol(res);
    setmedia(res);
    setloading(false)
        ...
        <div className="row px-4 pt-4">
          <div className="col-lg-5 col-md-12 d-flex align-items-center">
            <div className='w-100'>
              ...
              <h1 className="content"> {type} tv show <br /> to discover now</h1>
              <p className='text-muted'>most watched {type} tv show by day</p>
              <div className="input-box position-relative w-100">
                <input id='search_input' type="text" placeholder='Search for a tv show... ' className='form-control rounded-2' />
              ...
              <div id="carouselExampleCaptions" className="carousel slide" data-bs-ride="false">
                ...
                <div className="carousel-inner">
                  <div className="carousel-item active">
                    <Link to={`/tvdetalis/${(caursol[0].id) ? caursol[0].id : ""}`}>
                      ...
                      <div className="carousel-caption d-none d-md-block">
                        <h5 className='fw-bolde'>{caursol[0].name}</h5>
                        <p className='fw-bold fs-sm'>{caursol[0].overview.substring(0, 90)}</p>
                      </div>
                    </Link>
                  </div>
                  <div className="carousel-item">
                    <Link to={`/tvdetalis/${(caursol[1].id) ? caursol[1].id : ""}`}>
                      <img src={imgPath + caursol[1].backdrop_path} className="d-block w-100" alt="backdrop" />
                      <div className="carousel-caption d-none d-md-block">
                        <h5 className='fw-bolde'>{caursol[1].name}</h5>
                        <p className='fw-bold fs-sm'>{caursol[1].overview.substring(0, 90)}</p>
                      </div>
                    </Link>
                  </div>
                  <div className="carousel-item">
                    <Link to={`/tvdetalis/${(caursol[2].id) ? caursol[2].id : ""}`}>
                      <img loading="lazy" src={(caursol[2].backdrop_path ? imgPath + caursol[2].backdrop_path : nullphoto2)} alt="moviephoto" className='w-100' />
                      <div className="carousel-caption d-none d-md-block">
                        <h5 className='fw-bolde'>{caursol[2].name}</h5>
                        <p className='fw-bold fs-sm'>{caursol[2].overview.substring(0, 90)}</p>
                      </div>
                    </Link>
                  </div>
                  <div className="carousel-item">
                    <Link to={`/tvdetalis/${(caursol[3].id) ? caursol[3].id : ""}`}>
                      <img loading="lazy" src={(caursol[3].backdrop_path ? imgPath + caursol[3].backdrop_path : nullphoto2)} alt="moviephoto" className='w-100' />
                      <div className="carousel-caption d-none d-md-block">
                        <h5 className='fw-bolde'>{caursol[3].name}</h5>
                        <p className='fw-bold fs-sm'>{caursol[3].overview.substring(0, 90)}</p>
                      </div>
                    </Link>
                  </div>
                  <div className="carousel-item">
                    <Link to={`/tvdetalis/${(caursol[4].id) ? caursol[4].id : ""}`}>
                      <img loading="lazy" src={(caursol[4].backdrop_path ? imgPath + caursol[4].backdrop_path : nullphoto2)} alt="moviephoto" className='w-100' />
                      <div className="carousel-caption d-none d-md-block">
                        <h5 className='fw-bolde'>{caursol[4].name}</h5>
                        <p className='fw-bold fs-sm'>{caursol[4].overview.substring(0, 90)}</p>
                      </div>
                    </Link>
                  </div>
                </div>
                    ...
                    <div className="movie mx-2 position-relative">
                      <Link to={`/tvdetalis/${(e.id) ? e.id : ""}`}>
                        ...
                        <div className="movie-text mt-3 ms-1">
                          ...
                          <p className='text-muted fs-sm m-0'>{e.first_air_date}</p>
                        </div>
                      </Link>
                    </div>
                  </div>
